...
@register_prior("causal-sensitivity-msm-v2")
class CausalSensitivityMSMV2Prior(Prior):
    """
    Studio-compatible MSM causal sensitivity prior.

    Internally creates paper-style variables:
        X_context
        T
        output
        x_query
    ...
    Returns Studio-compatible fields:
        X      = packed context + query rows
        y      = scalar labels for query rows only
        n_ctx  = number of context rows

    Important Studio shape contract:
        X shape = (num_points + n_query_rows, d + 5)
        y shape = (n_query_rows,)

    Each causal query is represented as two query rows:
        bound_type = 0.0 -> lower bound row
        bound_type = 1.0 -> upper bound row

    Row format:
        [x features..., treatment, output, gamma, is_query, bound_type]

    Context rows:
        [x features..., treatment, observed_output, 0.0, 0.0, 0.0]

    Query rows:
        [x_query features..., t_query, 0.0, gamma, 1.0, bound_type]
    """
    ...
    def sample(
        ...
        **kwargs,
        ...
        # -------------------------------------------------
        # 1. Generate observed covariates X_context
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # U is internal only. It is not returned.
        ...
        output_norm = (output_raw - output_mean) / output_std
        ...
        base_x_norm = X_norm[query_indices]
        ...
        # -------------------------------------------------
        # 8. Build query rows and scalar query labels
        # -------------------------------------------------
        # Each query produces two rows:
        #   bound_type = 0.0 -> lower bound
        #   bound_type = 1.0 -> upper bound
        #
        # y is 1-D and contains labels only for query rows.
        ...
        for x_raw, x_norm in zip(base_x_raw, base_x_norm):
            ...
            for t_val in [0.0, 1.0]:
                for g in gamma_grid:
                    ...
                    # ONE row per bound.
                    # Row format:
                    # [x_query features..., t_query, output_placeholder, gamma, is_query, bound_type]
                    for bound_type, bound_val in ((0.0, lower), (1.0, higher)):
                        query_row = np.concatenate(
                            [
                                ...
                                np.array(
                                    [
                                        t_val,
                                        0.0,       # missing output placeholder
                                        g,
                                        1.0,       # is_query
                                        bound_type,
                                    ],
                        ...
                        query_feature_rows.append(query_row)
                        query_y_rows.append(bound_val)

        query_feature_rows = np.asarray(query_feature_rows, dtype=np.float32)
        query_y = np.asarray(query_y_rows, dtype=np.float32)

        # -------------------------------------------------
        # 9. Pack context rows
        # -------------------------------------------------
        # Context row format:
        # [X features..., T, output, gamma_placeholder, is_query, bound_type]
        #
        # gamma is 0 for context rows.
        # is_query is 0 for context rows.
        # bound_type is 0 for context rows.
        ...
        context_rows = np.concatenate(
            [
                ...
                T,
                output_norm,
                np.zeros((num_points, 1), dtype=np.float64),
                np.zeros((num_points, 1), dtype=np.float64),
                np.zeros((num_points, 1), dtype=np.float64),
            ],
            axis=1,
        ...
        # -------------------------------------------------
        # 11. Return Studio-compatible keys
        # -------------------------------------------------
        # y is query-only and 1-D.
        # n_ctx tells Studio where the context rows end.

        return {
            "X": X_packed,
            "y": query_y,
            "n_ctx": int(num_points),
        }
